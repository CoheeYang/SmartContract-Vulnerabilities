...
- **Consistency**: 所有节点看到的数据要一致，最新写入(write)必须立刻可读。即**所有服务器始终显示相同的数据**。当 Alice 的余额在服务器 A 上发生变化时，其他所有服务器会立即反映这一变化。
- **Availability:** 系统中的非失效节点都能响应每一个请求。**即使服务器崩溃，系统也能继续工作**。如果服务器 A 宕机，用户仍然可以通过服务器 B 和 C 访问账户。
- **Partition Tolerance**: 节点之间任意数量的信息在传输中遗失或者延迟都不会导致系统的停摆。**系统能够在网络故障将服务器分隔成孤立组时继续运行**。

**对于几乎所有系统，第三点的P几乎是不可或缺的，因为如果网络故障导致整个系统的失败通常是不能接受的，所以实际上所有的系统都会在前二者C和A中寻找平衡。**

而之所以这是一个不可能三角，其实按一个很简单的例子来看，如果我有一堆分布式节点，现在如果我将它们进行比如防火墙分离，即进行Partition，如果第三点满足，就意味着得在C或者A中进行选择，即两边的服务器要么放弃A，等待网络重新同步保证所有节点的数据都能保持一致。或者放弃C，显示可能不一致的信息，但是用户始终能够访问。

而倾向C的系统一般更加safe，这是因为所有写入的数据都会被读取，这也就意味着如double spending这种写入数据没有正确更新导致的问题无法存在。一般的银行系统都会保证C：**它们宁愿关闭系统也不愿显示错误的账户余额**。

而倾向A的系统则表现更具有Liveness，会更有活力，也就是说用户的请求总是会被响应，保证了系统能对交易进行及时的落块或者返回error。比如社交媒体平台：**它们宁愿让你发布内容（即使朋友无法立即看到）也不愿完全阻止你发布**。
