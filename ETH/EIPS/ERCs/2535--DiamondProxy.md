**EIP 2535（钻石标准）** ，这是以太坊上一种智能合约模块化架构标准 —— 不同于传统单合约 “臃肿代码” 的设计，它将合约功能拆分为多个独立 “切面（Facet）”，各切面仅负责特定功能，既降低了单合约代码复杂度，也方便后续功能升级（无需重新部署整个合约，仅需更新对应切面）。

![Facet reuse](https://eips.ethereum.org/assets/eip-2535/facetreuse.png)

其特点是所有的storage在DiamondProxy合约中初始好，之后用mapping指向多个impl合约，而不是一个proxy对应一个impl

比如下面的Amplify中Diamond实现。
...
contract SimplexDiamond is IDiamond {
    //作为proxy存储storage，一开始就进行storage的初始化
    constructor() {
