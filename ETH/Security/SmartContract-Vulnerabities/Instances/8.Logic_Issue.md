...
一个deposit后计算shares的函数，但是它却先收了用户的钱后，增加了`totalAsset`，再计算对应的shares，导致稀释了用户的share

[Hybra Finance](https://code4rena.com/reports/2025-10-hybra-finance#h-01-assets-deposited-before-calculating-shares-amount-to-mint-will-cause-users-to-mint-less-shares)
...
Dust 攻击，用户进行vote很小的投票权，之后poke本来应该更新随时间变化的所减少的投票权，但是由于poke中有一个0值revert检查，导致如果上一轮vote很少，下一轮投票权为0时，会导致更新的函数poke revert，从而让用户的其他投票权一直保持不变

[Hybra Finance](https://code4rena.com/reports/2025-10-hybra-finance#m-04-dust-vote-on-one-pool-prevents-poke)
...
这个函数应该发挥某样的作用，但是实际情况可能却和其完整功能不符合。也有前后口径不一致的问题



比如

[Hybra Finance](https://code4rena.com/reports/2025-10-hybra-finance#m-01-clfactory-ignores-dynamic-fees-above-10-and-silently-falls-back-to-default)中，期望池子的费用可以超过10%，但是view函数在超过10%后却返回默认的swap费用



还有比如

[Hybra Finance](https://code4rena.com/reports/2025-10-hybra-finance#m-02-users-emergency-withdrawing-will-lose-all-past-accrued-rewards)期望emergency withdraw能将用户的资金+奖励一并提取出来，但是实际却只提取了用户的本金，而奖励被卡住
